...
import { User } from "./colomns"

interface ApiUserDetailsDialogProps {
  user: User | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function ApiUserDetailsDialog({ user, open, onOpenChange }: ApiUserDetailsDialogProps) {
  if (!user) return null
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Complete user information from API
          </DialogDescription>
        </DialogHeader>
